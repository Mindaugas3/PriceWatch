     ...
     class PiguLt
     {
         ...
         private static void PrintData()
         {
             for (int i = 0; i < Links.Count(); i++)
             {
                 ...
                 CategoryForDB[i] = CategoryForDB[i].Substring(0,
                     CategoryForDB[i].LastIndexOf("/", CategoryForDB[i].LastIndexOf("/") - 1));
         ...
         private static async Task GetHtmlAsync(List<string> Links, string url)
         {
             ...
             else
             {
                 ...
                 foreach (string link in Links)
                 {
                     ...
                     try
                     {
                         Price.Add(htmlDocument.DocumentNode.Descendants("div").Where(node =>
                                 node.GetAttributeValue("class", "").Equals("product-price fl notranslate"))
                     ...
                     try
                     {
                         ...
                         Rating.Add(htmlDocument.DocumentNode.Descendants("div")
                             .Where(node => node.GetAttributeValue("class", "").Equals("rating")).FirstOrDefault()
                             .InnerText);
